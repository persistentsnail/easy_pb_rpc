#include <string>
#include <iostream>
#include <fstream>
#include <map>
#include <stdio.h>
#include <stdlib.h>

#include "common.h"

#define ALL_SERVICEs_CFG "servics.cfg"
using namespace std;

static map<string, unsigned int> ServiceName2IdTbl;

int RpcServiceStartup()
{
	ifstream in(ALL_SERVICEs_CFG);
	if (in.fail())
		ErrorRet("Open service cfg Read failed", -1);
	const int MAX_SERVICE_NAME_LEN = 1024;
	char ServiceName[MAX_SERVICE_NAME_LEN];
	unsigned int id = 1;
	while (in.getline(ServiceName, MAX_SERVICE_NAME_LEN)) {
		ServiceName2IdTbl[ServiceName] = id++;
	}
	return 0;	
}

unsigned int RpcServiceName2Id(const char *service_name)
{
	map<string, unsigned int>::iterator the_iter = ServiceName2IdTbl.find(service_name);
	if (the_iter == ServiceName2IdTbl.end())
		return 0;
	return the_iter->second;
}
